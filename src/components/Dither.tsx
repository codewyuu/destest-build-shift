...
function DitheredWaves({
  ...
  const [uniformsToPass, setUniformsToPass] = useState<WaveUniforms | null>(
    null
  )

  useEffect(() => {
    setUniformsToPass(waveUniformsRef.current)
  }, [])
...
export default function Dither({
  ...
  const effectiveColorNum = isDark
    ? Math.max(4, colorNum)
    : Math.max(6, colorNum)
  const effectivePixelSize = isDark ? pixelSize : Math.max(3, pixelSize)
  const effectiveWaveAmplitude = isDark
    ? waveAmplitude
    : Math.min(waveAmplitude, 0.22)
  const effectiveWaveFrequency = isDark
    ? waveFrequency
    : Math.min(waveFrequency, 2.8)
